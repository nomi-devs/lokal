...
Every domain module (`users`, `vendors`, `addresses`, `files`, `otp`, `products`, `refresh-tokens`, `wishlists`, `cart`, `orders`, `notifications`, `reviews`) follows the same shape — `users/` is the reference:
