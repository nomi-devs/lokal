...
| KYC Verification | `/admin/kyc-verification` | `adminApi.ts` (`listVendors`, `approveVendor`, `rejectVendor`) | `GET /admin/vendors?status=pending_approval`, approve/reject via `PUT /admin/vendors` — a moderation-queue view over the same Vendors collection, not a separate one, see below |
...
| FAQs | `/admin/faqs` | `adminApi.ts` (`listAdminFaqs`, `createAdminFaq`, `updateAdminFaq`, `deleteAdminFaq`) | `GET/POST/PUT/DELETE /admin/faqs` — `local-be/src/faqs/` also exposes a public, unauthenticated `GET /faqs` (active-only) for the mobile app, not used by the dashboard |
...
- **Settings is a generic key/value record, not live config** — `local-be/src/settings/` is a plain CRUD collection for visibility/record-keeping (`AdminSettingsController`); changing a value here does **not** change `local-be`'s actual runtime behavior, which stays driven by env vars (`registerAs`/`*.config.ts`, see Config below). `SettingsPage` is tabbed (`General` / `Support` / `Security`): the `General` tab holds the Commission card and the "Platform configuration" grid (both read/write this collection, grouped by category); `Security` is real (wired to `PUT /users/change-password` via `usersApi.ts`); the old "General" section from the original template (platform name, timezone…) was dropped — there's no backend field for any of it. The one exception is the platform commission rate, which `SettingsPage` also reads/writes (via `commissionApi.ts`, not `settingsApi.ts`) and which **is** live — see Commission below. `Support` is a dedicated `SupportInformationCard` (not the generic category grid) over 5 known keys — `supportEmail`, `supportPhone`, `whatsappNumber`, `websiteUrl`, `officeAddress` — under a `category: 'support'` this module added specifically to give that tab real backend fields; `local-be`'s `settings-seed.service.ts` (run via `npm run seed:admin`, see Commands above) creates those 5 rows with empty values if missing so the tab always has something to edit, same record-keeping-only caveat as the rest of this collection.
- **KYC Verification is a filtered view of Vendors, not a separate collection** — `local-be/src/vendors/` has no dedicated KYC schema; `KycVerificationPage` calls `adminApi.listVendors({ status: 'pending_approval' })` and approves/rejects through the same `AdminVendorsController` endpoints `VendorsPage` uses, same "view, not entity" precedent as Payments.

### Vendor dashboard
[src/pages/vendor/](dashboard/src/pages/vendor/) — Dashboard, Products, Orders, Earnings, Notifications, Store Profile, all scoped to the authenticated vendor. Reuses the same `DataTable`/`DynamicForm`/`DashboardLayout` primitives as the admin side rather than duplicating them; `VendorProducts` and its `VendorProductFormDialog` are the vendor-scoped counterpart to the admin `ProductsPage` (same fields, minus the vendor picker — `vendorId` is implicit from the logged-in user).

There's no `getVendorOrders(vendorId)`-style client-side lookup anymore — `local-be`'s `GET /vendor/orders` (`ordersApi.ts`'s `listVendorOrders()`) already scopes to the authenticated vendor server-side (resolved fresh from the JWT's user id on every request, not a cached `vendorId`), so every vendor page just calls it directly. `VendorDashboard` computes its stats/charts live from `listVendorOrders()` + `listMyProducts()` (`productsApi.ts`) so they stay consistent with the rest of the app; "top products" sorts by `Product.ratingCount` (real, server-maintained by the reviews module — see Public storefront browsing & reviews) rather than exact units sold, since order line items don't carry a per-item sales counter. `VendorEarnings` derives gross/commission/net per order from that same order's own `commissionPercentSnapshot` (frozen at checkout, not the platform's possibly-since-changed current rate — see Commission below) — there's no payout/bank-account tracking in `local-be`, so the old mock's bank-account panel doesn't exist here; "paid" vs "pending" payout status is a client-side label derived from `order.status === 'delivered'`, not a real payout record.
...
Every domain module (`users`, `vendors`, `addresses`, `files`, `otp`, `products`, `categories`, `banners`, `refresh-tokens`, `wishlists`, `cart`, `orders`, `commission`, `notifications`, `reviews`, `promo-codes`, `refunds`, `settings`, `faqs`) follows the same shape — `users/` is the reference:
...
### Commission
`commission/` (`CommissionService`/`PlatformCommission`) replaced a per-vendor `Vendor.commissionStructure.defaultPercentage` with one platform-wide rate — a single-document collection (`getOrDefault()` falls back to 15% if nothing's been set yet). `CommissionController` (`GET/PATCH /commission`, `JwtAuthGuard` + `RolesGuard`) lets `admin` or `vendor` read the current rate but only `admin` change it; `OrdersService.checkout()` is the sole caller of `getPercentage()`, snapshotting it onto every order as `commissionPercentSnapshot` (see Orders & checkout above). Unlike `settings/` below, this is real, load-bearing config — a `PATCH` here changes checkout math immediately. The dashboard's `commissionApi.ts` is read from two places: `SettingsPage` (admin, sets the rate) and `VendorStore` (vendor, views their own cut) — it has no dedicated page or route of its own.
