...
import {
  Store, Mail, Phone, Tag, MapPin, Star, FileText, User,
  Percent, Calendar, CheckCircle2, ShieldCheck, Package,
  ShoppingBag, Clock, XCircle, Truck, CheckCircle, ExternalLink,
} from "lucide-react";
...
// ── Helpers ───────────────────────────────────────────────────────────────────
const avatarColors = ["bg-violet-500", "bg-blue-500", "bg-emerald-500", "bg-amber-500", "bg-pink-500", "bg-teal-500"];
...
const approvalStyle: Record<VendorStatus, { text: string; bg: string; dot: string }> = {
  pending: { text: "text-amber-700 dark:text-amber-400", bg: "bg-amber-100 dark:bg-amber-900/30", dot: "bg-amber-400" },
  approved: { text: "text-emerald-700 dark:text-emerald-400", bg: "bg-emerald-100 dark:bg-emerald-900/30", dot: "bg-emerald-500" },
  rejected: { text: "text-red-700 dark:text-red-400", bg: "bg-red-100 dark:bg-red-900/30", dot: "bg-red-500" },
...
const productStatusKey: Record<ProductStatus, string> = { active: "active", inactive: "inactive", out_of_stock: "outOfStock" };
...
type OrderRow = { id: number; orderNumber: string; status: string; totalAmount: number; createdAt: string };
...
export default function VendorViewDialog({ open, onOpenChange, vendor }: Props) {
  ...
  const initials = vendor.nameEn.split(" ").map((n) => n[0]).slice(0, 2).join("");
  ...
  const orderRows: OrderRow[] = orders
    ...
    .filter((x): x is { order: typeof orders[number]; split: NonNullable<typeof x.split> } => !!x.split)
    .map(({ order, split }) => ({
      id: order.id,
  ...
  const tabs: { id: Tab; icon: React.ElementType; label: string; count?: number }[] = [
    ...
    { id: "products", icon: Package, label: t("vendors.view.tabs.products"), count: vendorProducts.length },
    { id: "orders", icon: ShoppingBag, label: t("vendors.view.tabs.orders"), count: orderRows.length },
    { id: "reviews", icon: Star, label: t("vendors.view.tabs.reviews"), count: reviewRows.length },
    { id: "documents", icon: FileText, label: t("vendors.view.tabs.documents"), count: documentRows.length },
  ];
  ...
  const productColumns: ColumnDef<Product>[] = [
    {
      ...
      render: (_, row) => (
        <div className="flex items-center gap-3 min-w-0">
          {row.images[0] ? (
            <img src={row.images[0]} alt={row.nameEn} className="w-8 h-8 rounded-md object-cover shrink-0" />
          ) : (
    ...
    { key: "price", header: t("products.list.columns.price"), render: (_, row) => `${row.price.base} ${row.price.currency}` },
    {
      key: "status",
      header: t("products.list.columns.status"),
      sortable: true,
      render: (v) => (
        <span className={cn("text-xs font-semibold px-2 py-0.5 rounded-full", productStatusStyle[v as ProductStatus])}>
  ...
  const orderColumns: ColumnDef<OrderRow>[] = [
    ...
    { key: "totalAmount", header: t("orders.columns.total"), sortable: true, align: "right", render: (v) => `${(v as number).toFixed(2)} KWD` },
  ...
  const documentColumns: ColumnDef<DocumentRow>[] = [
    ...
    { key: "uploadedAt", header: t("vendors.view.columns.uploadedAt"), sortable: true },
    {
      ...
      align: "right",
      render: (v) => (
        <a href={v as string} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 text-sm font-medium text-primary hover:underline">
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="p-0 max-w-5xl h-[85vh] flex flex-col gap-0">
        ...
        <div className="flex items-center gap-4 px-6 py-5 border-b shrink-0">
          <div className={cn("w-14 h-14 rounded-lg flex items-center justify-center text-lg font-bold text-white shrink-0", avatarBg)}>
            {initials}
          </div>
          ...
          <span className={cn("inline-flex items-center gap-1.5 text-xs font-semibold px-2.5 py-1 rounded-full shrink-0", approvalStyle[vendor.status].text, approvalStyle[vendor.status].bg)}>
          ...
          {activeTab === "details" && (
            <div className="rounded-xl border p-5">
              ...
              <div className="grid grid-cols-2 gap-x-12 gap-y-6">
                <InfoField icon={User} label={t("vendors.dialog.ownerName")} value={vendor.ownerName} />
                ...
                <InfoField icon={Tag} label={t("vendors.dialog.category")} value={vendor.category} />
                <InfoField icon={MapPin} label={t("vendors.dialog.city")} value={vendor.city} />
                <InfoField
                  icon={Percent}
                  label={t("vendors.dialog.commission")}
                  value={vendor.commission.type === "percentage" ? `${vendor.commission.value}%` : `${vendor.commission.value} KWD`}
                />
                {vendor.businessLicense && (
                  <InfoField icon={ShieldCheck} label={t("vendors.view.businessLicense")} value={vendor.businessLicense} />
                )}
                {vendor.taxId && (
                ...
                {vendor.approvedAt && (
                  <InfoField
                    ...
                    value={new Date(vendor.approvedAt).toLocaleDateString("en-US", { year: "numeric", month: "long", day: "numeric" })}
                  />
                )}
                {vendor.rating != null && (
                  <InfoField
